...
def record_test(
    category: str, test_name: str, success: bool, details: str = "", duration: float = 0
):
    """Record test result"""
...
def test_basic_functionality():
    ...
    start_time = time.time()
    try:
        from magic8_companion.analysis.gamma.gamma_runner import (
            run_batch_gamma_analysis,
        )
    ...
    try:
        ...
        if results and "net_gex" in results:
            record_test(
                ...
                "yahoo_provider",
                True,
                f"Successfully retrieved data via Yahoo",
                duration,
            )
        else:
            record_test(
                "data_providers", "yahoo_provider", False, "No data retrieved", duration
            )
        ...
        duration = time.time() - start_time
        record_test(
            "data_providers", "yahoo_provider", False, f"Exception: {str(e)}", duration
        )

    # Test IBKR provider with fallback
    logger.info("\nTest 4B: IBKR data provider")
    start_time = time.time()
    try:
        settings.data_provider = "ib"
        from magic8_companion.analysis.gamma.gamma_runner import run_gamma_analysis
        ...
        if results and "net_gex" in results:
            source = results.get("data_provider", "ib")
            record_test(
                "data_providers",
                "ib_provider",
                True,
                f"Data retrieved via {source}",
                duration,
            )
        else:
            record_test(
                "data_providers", "ib_provider", False, "No data retrieved", duration
            )
    except Exception as e:
        duration = time.time() - start_time
        record_test(
            "data_providers", "ib_provider", False, f"Exception: {str(e)}", duration
        )
    finally:
        settings.data_provider = original_provider
    ...
    try:
        ...
        if recommendations and gamma_included:
            ...
            record_test(
                ...
                "unified_combo_scorer",
                False,
                f"Gamma data not found in recommendations",
                duration,
            )
...
def test_edge_cases():
    ...
    try:
        ...
        record_test(
            ...
            "invalid_symbol",
            True,
            "Handled invalid symbol gracefully",
            duration,
        )
    except Exception as e:
        duration = time.time() - start_time
...
def generate_report():
    ...
    print(
        f"Success Rate: {test_results['summary']['passed']/max(test_results['summary']['total'], 1)*100:.1f}%"
    )
